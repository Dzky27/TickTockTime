...
// ===== END QUIZ EARLY (Individual) =====
function endQuizEarly() {
    if (!confirm('End quiz now? Your current score will be calculated and results will be shown.')) return;
    // Stop any ongoing timer interactions
    // Dismiss any open feedback popup
    const popup = document.getElementById('feedbackPopup');
    if (popup) popup.classList.add('hidden');
    // Show result with whatever score has been accumulated
    showResult();
}

// ===== END QUIZ EARLY (Group) =====
function endGroupQuizEarly() {
    if (!confirm('End group quiz now? Current scores will be shown on the leaderboard.')) return;
    // Stop the timer
    clearInterval(state.groupTimer);
    // Hide any open feedback popup
    const popup = document.getElementById('groupFeedback');
    if (popup) popup.classList.add('hidden');
    // Show group result with current scores
    showGroupResult();
}
  ...
  _wCheckpts.forEach((cp, idx) => {
    ...
    for (let bx = 0; bx < beamW; bx += bs) {
      for (let by = 0; by < beamH; by += bs) {
        ctx.fillStyle = ((Math.floor(bx/bs) + Math.floor(by/bs)) % 2 === 0) ? '#FFFFFF' : '#2D3436';
...
function _dFinish(W, H, roadTop, roadBot, roadH, ts) {
  ...
  for (let ox = 0; ox < bw; ox += bs) for (let oy = 0; oy < bh; oy += bs) {
    ctx.fillStyle = ((Math.floor(ox/bs) + Math.floor(oy/bs)) % 2 === 0) ? '#FFFFFF' : '#2D3436';
...
function rbBuildMCClock(q, container, player) {
  ...
  q.options.forEach(opt => {
    ...
    lbl.style.cssText = 'font-size:10px;font-weight:700;color:#636e72;';
...
function rbBuildArrange(q, container, player) {
  ...
  bLbl.style.cssText = 'font-size:10px;font-weight:700;color:#636e72;margin-bottom:3px;font-family:"Baloo 2",cursive;';
  ...
  aLbl.style.cssText = 'font-size:10px;font-weight:700;color:#636e72;margin:6px 0 3px;font-family:"Baloo 2",cursive;';
...
function rbBuildDrag(q, container, player) {
  ...
  hint.style.cssText = 'font-size:10px;color:#636e72;font-weight:600;text-align:center;font-family:"Baloo 2",cursive;';
